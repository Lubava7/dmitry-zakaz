...
## КАК ДОБАВИТЬ НОВЫЙ ПРОЕКТ ДЛЯ ФОТОГРАФИЙ

1. **Создай папку для нового проекта**

   - Открой папку `images/projects`.
   - Создай новую папку с именем по примеру уже существующих папок
   - Имя папки должно быть на английском, без пробелов и совпадать с полем `id` для этого проекта в коде (см. ниже).

2. **Добавь фотографии в папку проекта**

3. **Обнови файл manifest.json**

4. **Добавь информацию о проекте в код**

   - Открой файл `js/films.js`.
   - Найди массив с данными проектов ("const film_data = [...").
   - Добавь новый объект с данными о проекте, например:
     ```js
     {
       id: 'my_new_project', // имя папки проекта
       url: 'projects/my_new_project/1.jpg', // главное фото для карточки
       short_name: 'Название', // короткое название
       short_description: 'Описание', // короткое описание
       name: 'Полное название', // полное название
       description: 'Длинное описание проекта', // длинное описание
     }
     ```
   - Поле `id` должно совпадать с именем папки проекта!

5. **Сохрани все изменения**
   - После этого новый проект появится на сайте в разделе проектов.

**Если что-то не отображается — проверь:**

- Имя папки и поле `id` совпадают.
- Фото есть в папке и перечислены в `manifest.json`.
- Все имена файлов и папок — только английские буквы и цифры, без пробелов.
...
- Чтобы изменить текст на странице "О себе":
  1. Открой файл `about/about.html`.
...
- Чтобы добавить новые фото, видео или поменять текст:
  ...
  4. Если нужно изменить текст или порядок — дважды кликни на нужный файл (например, `js/script.js`, `about/about.html`, `js/videos.js`) и отредактируй его прямо в браузере через ISPmanager.
...
# КАК ПОМЕНЯТЬ ТЕКСТ ВО ВКЛАДКЕ (ЗАГОЛОВОК TAB)

- Чтобы изменить текст, который отображается во вкладке браузера (tab), нужно поменять содержимое тега `<title>` в нужном html-файле.
- Открой нужный файл (например, `portfolio/index.html`, `about/index.html`, `films/index.html`, `film/index.html`, `videos/index.html`, `video/index.html`, или `index.html` для главной).
- Найди строку вида:
  ```html
  <title>films</title>
  ```
- Замени текст внутри тега `<title> ... </title>` на нужный тебе (например, `<title>Мои проекты</title>`).
- Сохрани файл. Новый текст появится во вкладке браузера после обновления страницы.
